...
#' This takes a csv file containing replicate-average TMM expression 
#' counts by stages and plots a line plot of all the genes in the 
#' file after log transformation and centering. The function also
...
lineplot_stages <- function(file, gene_group = "", plot_mean = T){
  ...
  p1 <- ggplot() +
    ...
    ggtitle(paste(gene_group, "Gene expression across stages", sep = " ")) +
  ...
  if (!plot_mean) {
    ...
    stage_means_max <- stage_means[which(
      stage_means$stage_means == max(stage_means$stage_means)
      ), 1]
    stage_means_min <- stage_means[which(
      stage_means$stage_means == min(stage_means$stage_means)
      ), 1]
    cat("stage with highest mean expression level:", 
        as.character(stage_means_max), "\n")
    cat("stage with lowest mean expression level:", 
        as.character(stage_means_min), "\n")
  
    p1 <- p1 + geom_line(data = stage_means, 
                         aes(x = stages, y = stage_means, group = 1),
                         color = "black", 
                         size = 0.8)
    return(p1)
  }
